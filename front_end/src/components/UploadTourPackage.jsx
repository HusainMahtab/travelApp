...
function UploadTourPackage({onClose}) {
   ...
   const handleSubmit = async (e) => {
       ...
       try {
         const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/api/v1/tours/create_tour`, formData,{
           headers: {
         ...
         console.error('error while uploading Tour package', error);
     ...
     <div>
       ...
       <div className='fixed w-full top-0 left-0 right-0 bottom-0 flex justify-center items-center bg-slate-200 bg-opacity-80 z-50'>
        ...
        <div className='flex justify-between items-center'>
            <h1 className='text-lg font-bold'>Upload Tour Package</h1>
        ...
        <div className="w-full h-[250px] p-2 mx-auto overflow-hidden m-2">
          ...
          <button className='w-full rounded mt-5 p-2 border-2 border-white text-white font-bold hover:bg-[#9933ff] text-lg hover:text-white'>Upload</button>
       </form>
      </div>
     </div>
  )
